...
`tools/list` at `https://gateway.pipeworx.io/metmuseum/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/metmuseum_search_artworks \
  -H 'Content-Type: application/json' \
  -d '{"q":"sunflowers"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/metmuseum_search_artworks`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
